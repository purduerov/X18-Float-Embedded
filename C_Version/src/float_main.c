...
typedef struct __attribute__((packed))
{
    uint8_t command;
    uint16_t seq_num;
    uint8_t payload[12]; // Holds 3 floats
} packet_t;
...
const char *FloatStateNames[] = {
    "IDLE", "PROFILING", "PROFILE_DONE", "DUMPING_DATA"};
...
int main()
{
    ...
    while (true)
    {
        ...
        if (state == FLOAT_PROFILING)
        {
            // Take a depth sample periodically
            if (now - lastSampleTime >= SAMPLE_INTERVAL_MS && sampleIndex < TOTAL_PACKETS)
            {
                ...
                printf(">> Sample %u/%lu: Depth %.2f m\n",
                       sampleIndex + 1, TOTAL_PACKETS, recorded_depths[sampleIndex]);
        ...
        else if (state == FLOAT_PROFILE_DONE && !currentlyTransmitting)
        {
            ...
            {
                printf(">> Broadcasting DONE_PROFILE...\n");
        ...
        else if (state == FLOAT_DUMPING_DATA && !currentlyTransmitting)
        {
            ...
            if (now - lastTxTime >= 2000)
            {
                ...
                // Pack the specific recorded depth into the payload
                // The payload can hold 3 floats, we will leave the other two as 0.0f for now
                float data_to_send[3] = {recorded_depths[currentSeqNum - 1], 0.0f, 0.0f};
                memcpy(tx_pkt.payload, data_to_send, 12);
            ...
            else
            {
                ...
                if (len == sizeof(packet_t))
                {
                    ...
                    if (state == FLOAT_IDLE)
                    {
                        ...
                        {
                            printf(">> Received BEGIN_PROFILE. Diving for 10 seconds...\n");
                            state = FLOAT_PROFILING;

                            // Reset tracking variables for the new dive
                        ...
                        {
                            memcpy(pid_gains, rx_pkt.payload, 12);
                    ...
                    else if (state == FLOAT_PROFILE_DONE)
                    {
                        ...
                        {
                            printf(">> Received SEND_DATA command. Starting data dump...\n");
                    ...
                    else if (state == FLOAT_DUMPING_DATA)
                    {
                        if (rx_pkt.command == CMD_ACK && rx_pkt.seq_num == currentSeqNum)
                        {
                            ...
                            if (currentSeqNum > TOTAL_PACKETS)
                            {
