...
void data_logger_add_sample(uint16_t co, uint32_t time, float depth, float pressure, uint16_t actuator_pos, uint16_t target_actuator_pos) {
    ...
    data_buffer[data_count].pressure_kpa = pressure;
...
void data_logger_dump_csv(void) {
    printf("\n--- START DATA DUMP ---\n");
    printf("CompanyNumber,Time(ms),Depth(m),Pressure(kPa),ActuatorADC,TargetADC\n");
    for (size_t i = 0; i < data_count; i++) {
        printf("%u,%lu,%.2f,%.2f,%u,%u\n", 
               data_buffer[i].company_number,
               data_buffer[i].time_ms,
               data_buffer[i].depth_m,
               data_buffer[i].pressure_kpa,
