...
void on_radio_interrupt(void) { radio_event = true; }

int main() {
  stdio_init_all();
  sleep_ms(3000); // Wait for USB serial to stabilize

  printf("\n\n=======================================\n");
  printf("LoRa Radio Diagnostic: Surface-to-Surface\n");
#ifdef NODE_PING
  printf("Mode: SURFACE NODE A (Ping/Transmit First)\n");
  printf("Config: CS=%d, RST=%1d, EN=%d, IRQ=%d\n", PIN_CS, PIN_RST, PIN_EN,
         PIN_IRQ);
#else
  printf("Mode: SURFACE NODE B (Pong/Listen First)\n");
  ...
  while (1) {
    ...
    while (!radio_event &&
           (to_ms_since_boot(get_absolute_time()) - start_tx < 1000)) {
      tight_loop_contents();
    }
      ...
      while (to_ms_since_boot(get_absolute_time()) - start_wait < 1000) {
        if (radio_event) {
          ...
          if (len > 0) {
            ...
            printf("Got Reply: %s [RSSI: %.1f dBm, SNR: %.1f dB]\n",
                   (char *)buffer, rssi, snr);
            got_reply = true;
            break;
          }
        }
  ...
  while (1) {
    if (radio_event) {
      ...
      if (len > 0) {
        ...
        float rssi = radio_get_rssi();
        float snr = radio_get_snr();
        printf("Received: %s [RSSI: %.1f dBm, SNR: %.1f dB]\n", (char *)buffer,
               rssi, snr);
        // Send reply
        char reply[32];
